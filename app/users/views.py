...
from flask import Blueprint, render_template,redirect, url_for, render_template, flash, request, session
from flask_login import login_user, logout_user, current_user, login_required
from sqlalchemy import exc
from app.users.models import Users, SocialLogin, UserRegister, AsyncOperation, AsyncOperationStatus, ProviderName, TodoItem
from forms import LoginForm, RegisterForm
from app.users.oauth import OAuthLogin
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user_register is not None and bcrypt.check_password_hash(user_register.password, form.password.data):
            users = Users.query.filter_by(id=user_register.users_id).first()
...
def get_status():
    if 'async_operation_id' in session:
        ...
        print 'line 162', async_operation_id
...
def external_auth(provider):   
    ...
    try:
        ...
        print "line 191", social_login.social_login_id
    ...
    if social_login is None:
        if not current_user.is_active:
            ...
            db.session.commit()
            provider_name = ProviderName.query.filter_by(name=provider).first()
            print "line 207", provider_name.name 
            ...
            provider_name = ProviderName.query.filter_by(name=provider).first()
            print "line 215", provider_name.name 
            social_login = SocialLogin(social_login_id=social_id, name=username, email=email, users_id=user.id, provider_name_id=provider_name.id)
            db.session.add(social_login)
            db.session.commit()
            print "add a new social_login 219" 
    # user is already logged in via social and wants to connect more social accounts
        ...
        # existing user login
        social_login = SocialLogin.query.filter_by(social_login_id=social_id).first() 
        print "line 228", social_login.social_login_id 
    ...
    print "line 241", social_login.id
